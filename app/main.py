...
#----------------- Analysis on Second Dataset -------------------------------#
	...
	# -------------------- Heatmap ------------------- #
	m = folium.Map(location=[df2['lat'].mean(), df2['lng'].mean()], zoom_start=2, width='100%', height='100', tile="OpenStreetMap")
	...
	# HeatMapWithTime(data=heat_data).add_to(m)
	...
	if any(get_grouped_country):
	    ...
	    if any(group_city) == True:
	    	...
	    	for idx, row in group_city.iterrows():
			    ...
			    # Add a custom DivIcon marker to display AQI value inside the marker
			    folium.map.Marker(
		            location=[row['lat'], row['lng']],
		            icon=folium.DivIcon(html=f"""
		                <div style="
